...
from providers.groq.request import GROQ_DEFAULT_MAX_TOKENS
...
def test_build_request_body_defaults(groq_provider):
    """build_request_body returns model and falls back to GROQ_DEFAULT_MAX_TOKENS."""
    req = MockRequest(max_tokens=None)
    body = groq_provider._build_request_body(req)
    assert body["model"] == "llama-3.3-70b-versatile"
    assert body["max_tokens"] == GROQ_DEFAULT_MAX_TOKENS


def test_build_request_body_respects_max_tokens(groq_provider):
...
async def test_stream_response_text(groq_provider):
    ...
    with patch.object(
        ...
        events = [e async for e in groq_provider.stream_response(req)]
...
@pytest.mark.asyncio
async def test_stream_response_error(groq_provider):
    ...
    with patch.object(
        ...
        events = [
            e async for e in groq_provider.stream_response(req, request_id="GRQ1")
        ]

    assert any("Connection refused" in e for e in events)
    assert any("message_stop" in e for e in events)
    assert any("GRQ1" in e for e in events)
